...
namespace Microsoft.Xna.Framework.Content.Pipeline.Audio
{
    internal class DefaultAudioProfile : AudioProfile
    {
        ...
        {
            return  platform.IsPlatform("Android") ||
                    platform.IsPlatform("DesktopGL") ||
                    platform.IsPlatform("MacOSX") ||
                    platform.IsPlatform("NativeClient") ||
                    platform.IsPlatform("RaspberryPi") ||
                    platform.IsPlatform("Windows") ||
                    platform.IsPlatform("WindowsPhone") ||
                    platform.IsPlatform("WindowsPhone8") ||
                    platform.IsPlatform("WindowsStoreApp") ||
                    platform.IsPlatform("iOS");
        }

        public override void ConvertAudio(TargetPlatform platform, ConversionQuality quality, AudioContent content)
        {
            // Default to PCM data.
            var targetFormat = ConversionFormat.Pcm;
            if (quality != ConversionQuality.Best)
            {
                if (platform.IsPlatform("iOS") || platform.IsPlatform("MacOSX"))
                    targetFormat = ConversionFormat.ImaAdpcm;

                else if (   platform.IsPlatform("Windows") || 
                            platform.IsPlatform("WindowsPhone") || 
                            platform.IsPlatform("WindowsPhone8") || 
                            platform.IsPlatform("WindowsStoreApp"))
                {
            ...
            ConvertToFormat(content, targetFormat, quality, null);
        }

        public override void ConvertStreamingAudio(TargetPlatform platform, ConversionQuality quality, AudioContent content, ref string outputFileName)
        {
            // Most platforms will use AAC ("mp4") by default
            var targetFormat = ConversionFormat.Aac;

            if (    platform.IsPlatform("Windows") ||
                    platform.IsPlatform("WindowsPhone8") ||
                    platform.IsPlatform("WindowsStoreApp"))
                targetFormat = ConversionFormat.WindowsMedia;

            else if (platform.IsPlatform("DesktopGL"))
