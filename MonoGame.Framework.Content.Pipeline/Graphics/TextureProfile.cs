...
namespace Microsoft.Xna.Framework.Content.Pipeline.Graphics
{
    ...
    {
        private static readonly List<TextureProfile> Profiles = new List<TextureProfile>();

        static TextureProfile()
        {
            ...
            var assembly = typeof(TextureProfile).Assembly;
            foreach (var t in assembly.GetTypes())
            {
                if (!t.IsAbstract && t.IsSubclassOf(typeof(TextureProfile)))
                {
                    var profile = (TextureProfile)Activator.CreateInstance(t);
        ...
        public static TextureProfile ForPlatform(TargetPlatform platform)
        {
            ...
            throw new PipelineException("There is no supported texture profile for the '" + platform.Name + "' platform!");
        }
